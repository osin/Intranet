...
$img_dim = getimagesize($url);
$images = array();
$return_array['description'] = '';
if (is_array($img_dim)&& count) {
  list($width, $height, $type, $attr) = $img_dim;
  $images[] = array("img" => $url, "width" => $width, "height" => $height, 'area' => ($width * $height), 'offset' => 0);
}

if (count(explode('youtube', $url))>1) {
  $url_info = parse_url($url);
  $url_args = explode('&', ($url_info['query']));
  $url_info = explode('=', ($url_args[0]));
  $url_img = "http://img.youtube.com/vi/".$url_info[1].'/0.jpg';
  $img_dim = getimagesize($url_img);
  list($width, $height, $type, $attr) = $img_dim;
  $images[] = array("img" => $url_img, "width" => $width, "height" => $height, 'area' => ($width * $height), 'offset' => 0);
}
